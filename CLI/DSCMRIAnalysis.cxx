...
int DoIt( int argc, char * argv[], const T1 &, const T2 &)
{
  ...
  try 
    {
    TEValue = GetEchoTime(inputVectorVolume->GetMetaDataDictionary());
    }
  catch (itk::ExceptionObject &exc)
    {
    itkGenericExceptionMacro(<< exc.GetDescription() 
  ...
  if (!OutputMTTFileName.empty())
    {
    typename OutputVolumeWriterType::Pointer mttwriter = OutputVolumeWriterType::New();
    mttwriter->SetInput(quantifier->GetMTTOutput() );
    mttwriter->SetFileName(OutputMTTFileName.c_str() );
    mttwriter->SetUseCompression(1);
    mttwriter->Update();
    }

  if (!OutputCBFFileName.empty())
    {
    typename OutputVolumeWriterType::Pointer cbfwriter = OutputVolumeWriterType::New();
    cbfwriter->SetInput(quantifier->GetCBFOutput() );
    cbfwriter->SetFileName(OutputCBFFileName.c_str() );
    cbfwriter->SetUseCompression(1);
    cbfwriter->Update();
    }
